...
<script src="https://s3.amazonaws.com/stitch-sdks/js/bundles/5.0.0/stitch.js"></script>

  document.addEventListener("DOMContentLoaded", async function () {
    // Initialize Stitch app client
    const client = stitch.Stitch.initializeDefaultAppClient("your-stitch-app-id");

    // Log in anonymously (for demonstration purposes)
    await client.auth.loginWithCredential(new stitch.AnonymousCredential());

    // Get a reference to your MongoDB collection
    const db = client.getServiceClient(stitch.RemoteMongoClient.factory, "mongodb-atlas").db("Projects");
    const collection = db.collection("project");

    // Fetch project data from MongoDB
    const projects = await collection.find({}).toArray();

    // Do something with the fetched projects
    console.log(projects);

    // Log out (for demonstration purposes)
    ...
    function populateProjectsFromJSON(jsonData) {
      // ... Your existing populateProjectsFromJSON function

    function createFilterButtons(categories, containerClass) {
      // ... Your existing createFilterButtons function

    function createSelectFilter(categories) {
      // ... Your existing createSelectFilter function
    }
  });